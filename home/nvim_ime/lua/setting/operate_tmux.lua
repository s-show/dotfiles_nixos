...
local function send_prompt_tmux_pane()
...
local function scroll_src_pane(key)
  if file_exist(path_for_src_pane) and aitool_exist() then
    ...
    send_key_tmux_frontend(key)
  end
end

vim.keymap.set("n", "<C-s>", send_prompt_tmux_pane, { desc = "Send buf text to tmux pane" })
vim.keymap.set("i", "<C-s>", function()
  vim.cmd('stopinsert')
  send_prompt_tmux_pane()
end, { desc = "Send buf text to tmux pane" })
vim.keymap.set("n", "<C-g>q", function()
  vim.cmd('quit!')
end, { desc = "Send buf text to tmux pane & vim quit!" })
vim.keymap.set("n", "<Up>", function() send_key_tmux_frontend('Up') end, { desc = "Send <up> cursor to tmux pane" })
vim.keymap.set("n", "<Down>", function() send_key_tmux_frontend('Down') end, { desc = "Send <down> cursor to tmux pane" })
vim.keymap.set("n", "<C-g><ESC>", function() send_key_tmux_frontend('Escape') end,
  { desc = "Send <Escape> cursor to tmux pane" })
vim.keymap.set("n", "<C-g>c", function() send_key_tmux_frontend('C-c') end, { desc = "Send <Ctrl-c> to tmux pane" })
vim.keymap.set("n", "<C-g>u", function() send_key_tmux_frontend('C-u') end, { desc = "Send <Ctrl-u to tmux pane" })
vim.keymap.set("n", "<Enter>", function() send_key_tmux_frontend('Enter') end, { desc = "Send <Enter> to tmux pane" })
vim.keymap.set("n", "<BS>", function() send_key_tmux_frontend('BSpace') end, { desc = "Send <BackSpace> to tmux pane" })
